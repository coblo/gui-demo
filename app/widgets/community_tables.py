...
class PermissionModel(QAbstractTableModel):
    ...
    def __init__(self, parent, perm_type=Permission.MINE):
        ...
        self._fields = ('Alias', 'Address', 'Last Mined' if perm_type == Permission.MINE else 'Last Voted',
                        'Last 24h', 'Revokes', 'Action')
            ...
            for perm in perms_data:
                for pending in perm['pending']:
                    if pending['startblock'] == 0 and pending['endblock'] == 0:
                        if Profile.get_active().address in pending['admins'] and perm[
                            'address'] not in self.already_voted:
