...
class TransactionTableView(QTableView):
    ...
    def create_table_buttons(self):
        ...
        for row in range(0, self.table_model.rowCount() - 1):
            self.openPersistentEditor(self.table_model.index(row, 5))
        if not self.table_model.all_transactions_shown:
            self.setItemDelegateForColumn(2, ButtonShowDelegate(self))
            self.openPersistentEditor(self.table_model.index(self.table_model.rowCount() - 1, 2))
...
class ButtonShowDelegate(QStyledItemDelegate):
    def __init__(self, parent):
        QStyledItemDelegate.__init__(self, parent)

    def createEditor(self, parent, option, idx):
        btn = QPushButton("Show more transactions", parent)
        btn.setStyleSheet("QPushButton {background-color: #0183ea; margin: 8 4 8 4; color: white; width: 45px}")
        btn.setFont(QFont("Roboto", 12))
        btn.setCursor(QCursor(Qt.PointingHandCursor))
        btn.clicked.connect(self.on_show_more_clicked)
        return btn

    def on_show_more_clicked(self):
        self.parent().table_model.limit += 100
        self.parent().table_model.refreshData()
...
class TransactionHistoryTableModel(QAbstractTableModel):
    ...
    def __init__(self, parent=None):
        ...
        self.limit = 100
        self.all_transactions_shown = False
        self.refreshData()
    ...
    def rowCount(self, parent=None, *args, **kwargs):
        return len(self.txs) if self.all_transactions_shown else len(self.txs) + 1
    ...
    def refreshData(self):
        ...
        with data_session_scope() as session:
            self.txs = WalletTransaction.get_wallet_history(session, self.limit)
        self.endResetModel()
        self.sort(self.sort_index, self.sort_order)
        if hasattr(self.parent, "table_model"):
            self.parent.create_table_buttons()
        with data_session_scope() as session:
            if session.query(WalletTransaction).count() == len(self.txs) and not self.all_transactions_shown:
                self.all_transactions_shown = True
